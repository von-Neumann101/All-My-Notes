...
> 
> Q：**B是否越大越好呢？**
> A：并非，虽然较大的块能利用程序中可能存在的**空间局部性**，但是如果给定缓存的大小，块越大那么行数就越小，这又会影响**时间局部性**。同时，块越大他的**不命中惩罚**就越大
## Cache Write
...
1. 步长为1的引用，因为Cache的块会把所需要地址周围的很多数据（B-对齐）都放进去
